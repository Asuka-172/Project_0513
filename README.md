...
一个展示 Unreal Engine 5 高级插件开发能力的综合性编辑器工具，集成了自定义 Slate UI、后处理效果系统、网络通信（TCP/UDP）、实时性能监控、资产管理与批量处理等多种模块。该项目可作为 TA/引擎开发岗位的技术作品集。
...
- **SBatchAssetTool**：集成三种核心操作模式：
  - **批量复制**：将源目录的资产复制到目标目录，自动添加后缀，检测同名避免覆盖。
  - **批量重命名**：支持添加前缀、添加后缀、按序号重命名三种策略。
  - **纹理压缩**：批量修改纹理的压缩格式（BC1/BC3/BC5/BC7）、Mip 生成设置和 sRGB 开关。
  - **预览功能**：执行前展示所有受影响的资产及目标名称。
  - **异步分帧执行**：每帧处理一个资产，编辑器保持响应，进度条实时更新。
  - **撤销支持**：重命名和压缩操作可通过 Ctrl+Z 撤销（使用 `FScopedTransaction` + `Modify()`）。
...
- **模块化架构**：每个功能（颜色选择、后处理、网络、性能、资产处理）都封装为独立的控件或类，通过插件模块统一调度，利于维护和扩展。
...
2. 在编辑器菜单栏中点击 `Window` → `My First Tool`，展开子菜单：
   ...
   - **Asset Statistics**：打开资产统计窗口，点击 Refresh 查看项目资产数量。
   - **Batch Asset Tool**：打开批量资产处理工具，支持复制、重命名、纹理压缩。
...
│ ├── SBatchAssetTool.h/cpp # 批量资产处理工具（复制/重命名/压缩）
│ ├── Network/
